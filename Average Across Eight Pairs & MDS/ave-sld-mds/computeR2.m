...
for iPair = 1:8
    for i = 1:18
            ...
            r = corr([mat1,mat2]);
            R2 = r.^2;
            R2acr = [R2(149:296,1:148),R2(1:148,149:296)];
            R2wit = [R2(1:148,1:148),R2(149:296,149:296)];
            R2acrnew = reshape(R2acr,148*296,1);
            R2witnew = reshape(R2wit,148*296,1);
            R2mat(:,i) = R2witnew;
            R2mat(:,i+18) = R2acrnew;
    end
        rr = corr(R2mat);
...
title('Within and Across Subjects')
set(gca,'xtick',[3.5,9.5,15.5,21.5,27.5,33.5]);set(gca,'ytick',[3.5,9.5,15.5,21.5,27.5,33.5]);set(gca,'FontSize',12);
