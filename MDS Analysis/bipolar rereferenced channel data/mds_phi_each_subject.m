...
for i = 1:63
    labels{1,i+63} = ['B' num2str(i)];
end
%TM
%load phi values
data_tm = load('brch-diff-TM-170821.mat');
data_tm = data_tm.tm_tri;
datatemp = data_tm';
%remove diagonal to get maximum and minimum 
nodiag = reshape(datatemp(~eye(size(datatemp))), 126*125, [])';
maximum = max(nodiag(:));
minimum = min(nodiag(:));
%make phi 0 to 1
len = maximum - minimum;
...
%RV
%load phi values
data_rv = load('brch-diff-RV-170821.mat');
data_rv = data_rv.rv_tri;
datatemp = data_rv';
%remove diagonal to get maximum and minimum 
nodiag = reshape(datatemp(~eye(size(datatemp))), 126*125, [])';
maximum = max(nodiag(:));
...
%OC
%load phi values
data_oc = load('brch-diff-OC-170821.mat');
data_oc = data_oc.oc_tri;
datatemp = data_oc';
%remove diagonal to get maximum and minimum 
nodiag = reshape(datatemp(~eye(size(datatemp))), 126*125, [])';
maximum = max(nodiag(:));
...
plot(Y_tm(1:63,1),Y_tm(1:63,2),'co',Y_tm(64:126,1),Y_tm(64:126,2),'go')
set(gca,'FontSize',16);
legend('Participant 1','Participant 2','location','North');grid on;title('Dissimilarity')
%text(Y_tm(:,1)+0.005,Y_tm(:,2),labels);
saveas(1,'1.jpg')

figure(2);
[Y_rv,e_rv] = cmdscale(dissimilarity_rv,2);
plot(Y_rv(1:63,1),Y_rv(1:63,2),'co',Y_rv(64:126,1),Y_rv(64:126,2),'go')
set(gca,'FontSize',16);
legend('Participant 1','Participant 2','location','North');grid on;title('Dissimilarity')
...
plot(Y_oc(1:63,1),Y_oc(1:63,2),'co',Y_oc(64:126,1),Y_oc(64:126,2),'go')
set(gca,'FontSize',16);
legend('Participant 1','Participant 2','location','North');grid on;title('Dissimilarity')
